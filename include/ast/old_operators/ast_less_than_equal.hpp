...
class LessThanEqual : public Node{
    ...
    LessThanEqual(Node* left, Node* right): leftOperand_(left), rightOperand_(right){};
    ...
    void EmitRISC(std::ostream &stream, Context &context) const override {
        ...
        if (leftOperand_->getType() == "constant" && rightOperand_->getType() == "constant") {
            ...
            if (leftOperand_->getType() == "constant") {
                ...
                stream << "slti " << dst << "," << dst << "," << leftOperand_->getVal() << std::endl;
                ...
                stream << "slti " << dst << "," << dst << "," << rightOperand_->getVal() + 1 << std::endl;
            ...
            stream << "sgt " << dst << "," << dst << "," << tmp << std::endl;
            stream << "xori " << dst << "," << dst << "," << 1 << std::endl;
            context.dst = leftOperand_->getId();
